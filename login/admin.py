from django import forms
from django.contrib import admin
from django.contrib.auth.models import Group
from django.contrib.auth.admin import UserAdmin as BaseUserAdmin
from django.contrib.auth.forms import ReadOnlyPasswordHashField
from django.core.exceptions import ValidationError
from .models import MyUser
from catalog.models import Product


class UserCreationForm(forms.ModelForm):
    """Форма для создания нового пользователя. Включает все обязательные поля
    и два поля для пароля."""

    password1 = forms.CharField(label="Password", widget=forms.PasswordInput)
    password2 = forms.CharField(
        label="Password confirmation", widget=forms.PasswordInput
    )

    class Meta:
        model = MyUser
        fields = ["phone_number", "first_name", "last_name"]

    def clean_password2(self):
        # Проверка совпадения паролей
        password1 = self.cleaned_data.get("password1")
        password2 = self.cleaned_data.get("password2")
        if password1 and password2 and password1 != password2:
            raise ValidationError("Passwords don't match")
        return password2

    def save(self, commit=True):
        # Сохранение пароля в хешированном виде
        user = super().save(commit=False)
        user.set_password(self.cleaned_data["password1"])
        if commit:
            user.save()
        return user


class UserChangeForm(forms.ModelForm):
    """Форма для обновления пользователя. Поле пароля заменяется на
    отображение хеша только для чтения."""

    password = ReadOnlyPasswordHashField()

    class Meta:
        model = MyUser
        fields = ["phone_number", "password", "first_name", "last_name", "is_active", "is_admin"]


class UserAdmin(BaseUserAdmin):
    # Формы для создания и изменения
    form = UserChangeForm
    add_form = UserCreationForm

    # Отображение в списке пользователей
    list_display = ["phone_number", "first_name", "last_name", "is_admin"]
    list_filter = ["is_admin"]

    # Поля для просмотра и редактирования
    fieldsets = [
        (None, {"fields": ["phone_number", "password"]}),
        ("Personal info", {"fields": ["first_name", "last_name"]}),
        ("Permissions", {"fields": ["is_admin"]}),
    ]

    # Поля для формы создания пользователя
    add_fieldsets = [
        (
            None,
            {
                "classes": ["wide"],
                "fields": ["phone_number", "first_name", "last_name", "password1", "password2"],
            },
        ),
    ]

    search_fields = ["phone_number", "first_name", "last_name"]
    ordering = ["phone_number"]
    filter_horizontal = []


# Регистрация нового UserAdmin
admin.site.register(MyUser, UserAdmin)
# Отмена регистрации Group, так как мы не используем стандартные разрешения Django
admin.site.unregister(Group)
admin.site.register(Product)